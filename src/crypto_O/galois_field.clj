...
;;
;; again, all hail http://www.samiam.org/galois.html
;;
...
(defn multiply [a b]
  (:p (reduce (fn [memo _] (multiply-inner-loop memo))
              {:p 0 :a a :b b}
              (range 8))))

(defn multiply-inverse [index]
  (if (= 0 index)
    0
    (atable (- 255 (ltable (int index))))))

(defn multiply-inner-loop [abp]
  (let [{:keys [a b p]} abp
        a-prime (bit-and 0xFF (bit-shift-left a 1))]
    {:a (if (bit-test a 7) (bit-xor a-prime 0x1b) a-prime)
     :b (bit-shift-right b 1)
     :p (if (odd? b) (bit-xor p a) p)}))

(defn rcon [input]
  (if (= 0 input)
    0
    (reduce (fn [c _] (multiply c 2))
            1
            (range input 1 -1))))
...
(def ltable [0x00 0xff 0xc8 0x08 0x91 0x10 0xd0 0x36 
